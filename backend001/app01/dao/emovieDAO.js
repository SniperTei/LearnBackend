...
const getMovieList = (currentPage, pageSize, condition) => {
  return new Promise((resolve, reject) => {
    ...
    let sql = `SELECT * FROM tbl_emovies tm where tm.is_deleted = 0`;
    if (condition.title) {
    ...
    if (condition.tags) {
      sql += ` AND tm.tag like '%${condition.tags}%'`;
    }
    // if (condition.genre) {
...
const getMovieCount = (condition) => {
  return new Promise((resolve, reject) => {
    let sql = `SELECT COUNT(*) as count FROM tbl_emovies tm where tm.is_deleted = 0`;
    if (condition.title) {
      sql += ` AND tm.title like '%${condition.title}%'`;
    }
    if (condition.actor) {
...
const getMovie = (movieId) => {
  return new Promise((resolve, reject) => {
    let sql = `SELECT * FROM tbl_emovies tm where tm.movieId = '${movieId}'`;
    pool.query(sql, (err, result) => {
      if (err) {
        console.error('An error occurred:', err);
        reject(err);
...
const addMovie = (movie) => {
  return new Promise((resolve, reject) => {
    ...
    let sql = `INSERT INTO tbl_emovies SET ?`;
    // Execute
...
const updateMovie = (movie) => {
  return new Promise((resolve, reject) => {
    let sql = `UPDATE tbl_emovies SET ? WHERE movieId = '${movie.movieId}'`;
    // Execute
    pool.query(sql, movie, (err, result) => {
      if (err) {
        console.error('An error occurred:', err);
...
const physicalDeleteMovie = (movie) => {
  return new Promise((resolve, reject) => {
    let sql = `DELETE FROM tbl_emovies WHERE movieId = ?`;
...
const logicDeleteMovie = (movie) => {
  return new Promise((resolve, reject) => {
    let sql = `UPDATE tbl_emovies SET is_deleted = 1 WHERE movieId = ?`;
    pool.query(sql, [movie.movieId], (err, result) => {
      if (err) {
        console.error('An error occurred:', err);
        reject(err);
